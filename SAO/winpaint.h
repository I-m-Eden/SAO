...
bool checkprev(LPCSTR wname);						//判断是否有先前实例
...
void setmousexy(int x, int y);							//设置鼠标在绘图区域中坐标xy
void setmousexy(POINT p);								//设置鼠标在绘图区域中坐标点
...
void Gdot(int x, int y, BYTE&r, BYTE&g, BYTE&b);		//得到像素点颜色(beginPdot后使用)
